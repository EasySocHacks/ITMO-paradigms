...
(defn Sinh [expression] (CommonOperation.
  (fn [x] (Math/sinh x))
  "sinh"
  (fn [diffVariable] (Multiply (Cosh expression) (diff expression diffVariable)))
  (list expression)))

(defn Cosh [expression] (CommonOperation.
  (fn [x] (Math/cosh x))
  "cosh"
  (fn [diffVariable] (Multiply (Sinh expression) (diff expression diffVariable)))
  (list expression)))

(def getOperation {
  '+ Add '- Subtract '* Multiply '/ Divide 'negate Negate 'square Square 'sqrt Sqrt 'sinh Sinh 'cosh Cosh
  })
