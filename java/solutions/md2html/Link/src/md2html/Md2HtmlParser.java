...
public class Md2HtmlParser {
	...
	private StringBuilder parseText(Md2HtmlOperationType expectedOperation) throws Md2HtmlException {
		...
		while (source.getChar() != source.END) {
			...
			if (expectedOperation == Md2HtmlOperationType.LINK_HREF) {
				if (source.getChar() == ')') {
					source.nextChar();
					return text;
				}
			...
			if (source.getChar() == '*') {
				...
				source.nextChar();
				text.append(parseText(expectedOperation.LINK_INPUT));
			} else if (source.getChar() == ']') {
				source.nextChar();
				if (expectedOperation == Md2HtmlOperationType.LINK_INPUT) {
					source.nextChar();
					StringBuilder linkHref = parseText(expectedOperation.LINK_HREF);
					
					return new StringBuilder("<a href='" + linkHref + "'>" + text + "</a>");
				} else {
					text.append("]");
				}
			} else {
